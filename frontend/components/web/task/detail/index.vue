<template>
    <div v-if="task" class="min-h-full bg-[var(--bg-surface)] p-6">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-6 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-[var(--text-primary)] flex items-center gap-3">
                            งาน #{{ task.id }}
                            <span class="px-2.5 py-0.5 rounded-full text-sm font-medium border"
            ...
            <TaskSatisfactionCard v-if="canRateThisTask || isSummarySaved" :isEmployee="isEmployee"
                :has-rated-this-task="hasRatedThisTask" :average-rating="averageRating" :loading="ratingLoading" :isMobile="isMobile"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Waiting State -->
                    <section v-if="!task.id"
                        class="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-subtle)] p-6 shadow-sm">
                        <div class="rounded-xl bg-[var(--color-primary-500)] px-4 py-3 shadow-md flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <img src="https://img.icons8.com/color/48/hourglass-sand-top.png" alt="รอรับเรื่อง"
                                    class="w-5 h-5 shrink-0" />
                                <div class="flex flex-col">
                                    <span class="text-xs text-[var(--color-primary-100)]">สถานะงาน</span>
                            ...
                            <span class="inline-flex items-center rounded-full bg-[var(--text-inverse)]/15 px-3 py-1 text-xs font-medium text-[var(--text-inverse)]">
                                กำลังจะเริ่มงาน
                            </span>
                        </div>
                    </section>
                ...
                <div class="space-y-6">
                    <TaskTimeLine v-if="task.id" :status="task.status" :currentStep="task.currentStep ?? 0"
        ...
        <ResponsiveModal :model-value="statusModal" @update:model-value="handleClose">
            <StatusModal :status="statusMessage.status" :title="statusMessage.title"
                :description="statusMessage.description" :needRefresh="statusMessage.needRefresh" @close="handleClose" />
        </ResponsiveModal>
