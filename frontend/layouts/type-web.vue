<template>
  <div
    class="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 text-slate-900 flex"
  >
    <!-- Sidebar (desktop) -->
    <aside
      class="hidden md:flex md:flex-col w-64 bg-white/90 backdrop-blur border-r border-slate-200/60 shadow-sm"
    >
      <!-- Logo / Brand -->
...
const toggleNotif = () => {
  ...
  if (next) {
    // ถ้าเปิดแจ้งเตือน ให้ปิด user menu
    userMenuOpen.value = false;
  }
};
...
const toggleUserMenu = () => {
  ...
  if (next) {
    // ถ้าเปิด user menu ให้ปิดแจ้งเตือน
    notifOpen.value = false;
  }
};
