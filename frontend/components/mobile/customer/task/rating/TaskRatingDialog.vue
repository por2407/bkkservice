<template>
  <div class="relative w-full rounded-t-3xl bg-white p-5 pb-5 shadow-lg">
    ...
    <div class="mb-3 flex items-center justify-between">
      ...
      <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500"
    ...
    <div class="mb-3 rounded-2xl bg-slate-50 px-3 py-2">
      <div class="flex flex-wrap justify-between gap-y-1 text-[10px] font-medium">
    ...
    <div class="max-h-[55vh] space-y-3 overflow-y-auto pr-1">
      <div v-for="(item, index) in ratingItems" :key="item.id"
        ...
        <div class="mb-1 flex items-center justify-between">
          ...
          <span v-if="tempRatingScores[item.id]" class="text-[11px] font-medium text-amber-600">
        ...
        <div class="flex gap-1.5">
          <button v-for="n in 5" :key="n" type="button"
            ...
            :disabled="hasRatedThisTask || ratingLoading" @click="!hasRatedThisTask && setTempScore(item.id, n)">
