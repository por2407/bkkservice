<template>
  <div class="relative w-full rounded-t-[30px] bg-white px-4 pt-4 pb-6 shadow-lg">
    <button type="button"
    ...
    <div v-if="previewMedia" class="mt-4 flex items-center justify-center">
      <div
        class="flex w-full max-w-[640px] max-h-[70vh] items-center justify-center overflow-hidden rounded-2xl bg-secondary-900/5 px-2 py-2">
        ...
        <video v-else :src="previewMedia.url" controls autoplay
          class="h-full max-h-[66vh] w-full max-w-full object-contain"></video>
      </div>
    </div>

    <div v-else
      class="mt-6 flex min-h-[220px] flex-col items-center justify-center rounded-2xl border border-dashed border-error-200 bg-error-50/80 px-4 text-center">
      <div class="mb-3 flex h-11 w-11 items-center justify-center rounded-2xl bg-white shadow-sm">
        <AlertOctagon class="h-6 w-6 text-error-500" />
      </div>

      <p class="text-[13px] font-semibold text-error-700">
        ไม่มีสื่อสำหรับแสดงตัวอย่าง
      </p>
      <p class="mt-1 max-w-xs text-[11px] leading-relaxed text-error-600">
        หน้าต่างตัวอย่างนี้ควรเปิดเมื่อเลือกรูปภาพหรือวิดีโอจากงานแล้ว
        แต่ตอนนี้ระบบไม่ได้รับข้อมูลสื่อเข้ามา
      </p>

      <p class="mt-2 max-w-xs text-[10px] text-error-500/80">
      ...
      <button type="button"
        class="mt-3 inline-flex items-center gap-1 rounded-full border border-error-200 bg-white px-3 py-1.5 text-[11px] font-medium text-error-600 shadow-sm"
        @click="$emit('close')">
        <X class="h-3.5 w-3.5" />
