  ...
  <section class="rounded-2xl bg-white p-3.5 shadow-sm">
    <div class="mb-2 flex items-center justify-between">
      <div class="inline-flex items-center gap-2 text-sm font-semibold text-secondary-900">
        <ImageIcon class="h-4 w-4 text-info-500" />
    ...
    <div class="flex gap-2 overflow-x-auto pb-1">
      <button v-for="(item, index) in media" :key="index" type="button"
        class="relative h-32 min-w-[140px] overflow-hidden rounded-2xl bg-secondary-100 focus:outline-none"
        ...
        <!-- วิดีโอ -->
        <video v-else :src="item.url" class="h-full w-full object-cover" muted playsinline></video>
