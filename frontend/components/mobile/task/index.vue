<template>
  ...
  <div class="min-h-full pt-4 pb-6 transition-colors duration-300" :class="[pageBgClass, isMobile ? 'px-3' : 'px-4']">
    <!-- top header / hero -->
    <TasksHeader :taskCount="filteredTasks.length">
      <div class="flex justify-center gap-2">
        ...
        <button type="button" @click="taksFilter.setActiveFilter('in_progress')"
          ...
          :class="taksFilter.activeFilter === 'in_progress'
            ? filterStyles.in_progress
            : 'bg-white/10 text-primary-50/90 opacity-80'
            ">
          <Clock class="h-5 w-5" />
          <div>
        ...
        <button type="button" @click="taksFilter.setActiveFilter('done')"
          class="w-1/3 flex items-center gap-2 rounded-2xl px-3.5 py-2.5 backdrop-blur transition active:scale-[0.98] border-2 border-white/70"
          :class="taksFilter.activeFilter === 'done'
            ? filterStyles.done
            : 'bg-white/10 text-primary-50/90 opacity-80'
            ">
        ...
        <button v-if="!isEmployee" type="button" @click="taksFilter.setActiveFilter('all')"
          class="w-1/3 flex items-center gap-2 rounded-2xl px-3.5 py-2.5 backdrop-blur transition active:scale-[0.98] border-2 border-white/70"
          :class="taksFilter.activeFilter === 'all'
            ? filterStyles.all
            : 'bg-white/10 text-primary-50/90 opacity-80'
            ">
    ...
    <section :class="isMobile ? 'sticky -top-3 z-30' : ''">
      <div :class="[
        ...
        <div :class="['pt-2 pb-2', isMobile ? 'px-3' : 'px-4']">
          <div class="relative">
            <!-- icon ซ้าย -->
            <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-primary-500/70">
            ...
            <input v-model="searchQuery" type="text" inputmode="search"
              ...
              class="w-full rounded-2xl border border-primary-100 bg-white/90 py-2 pl-9 pr-8 text-sm text-secondary-800 shadow-sm outline-none ring-0 placeholder:text-secondary-400 focus:border-primary-400 focus:bg-white focus:shadow-[0_10px_24px_rgba(15,23,42,0.08)] focus:ring-2 focus:ring-primary-200" />

            <!-- ปุ่มล้างข้อความ -->
            <button v-if="searchQuery" type="button" @click="searchQuery = ''"
              class="absolute inset-y-0 right-0 flex items-center pr-3 text-secondary-400 active:scale-95">
              <X class="h-4 w-4" />
            </button>
          </div>
        </div>
    ...
    <!-- list -->
    <TaskList v-if="!loading && filteredTasks.length > 0" :TaskData="visibleTasks" :isCustomer="isCustomer"
      :isDealer="isDealer" />

    <!-- sentinel สำหรับ infinite scroll -->
    <div ref="infiniteScrollTrigger" v-show="!loading && canLoadMore && filteredTasks.length > 0"
      class="h-10 w-full flex items-center justify-center text-xs text-secondary-400">
      เลื่อนลงเพื่อโหลดงานเพิ่ม...
    </div>

    <!-- skeleton ตอนโหลดครั้งแรก -->
    <section v-if="loading" class="mt-4 space-y-3" aria-busy="true" aria-live="polite">
      <div class="flex items-center justify-center gap-2 text-primary-700/80 text-sm">
        <span class="spinner"></span>
        <span>กำลังโหลดรายการงาน...</span>
      </div>

      <div v-for="i in 3" :key="i"
        class="skeleton-card rounded-2xl border border-primary-100 bg-white/80 p-3 shadow-sm">
    ...
    <!-- ไม่มีงาน -->
    <section v-else-if="!loading && filteredTasks.length === 0"
...
import TasksHeader from "@/components/mobile/task/TasksHeader.vue";
import TaskList from "@/components/mobile/task/TaskList.vue";
