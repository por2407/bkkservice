<template>
  ...
  <section class="rounded-2xl bg-white p-3.5 shadow-sm">
    <div class="mb-3 flex items-center justify-between">
      <div class="inline-flex items-center gap-2 text-sm font-semibold text-secondary-900">
    ...
    <div class="relative">
      <!-- เส้นตั้ง -->
      <div class="pointer-events-none absolute left-[13px] top-3 bottom-3 w-[2px] bg-secondary-200"></div>

      <div v-for="step in stepsDisplay" :key="step.key" class="relative flex gap-3 pb-4 last:pb-0">
        <div class="relative mt-1 flex h-7 w-7 items-center justify-center rounded-full border-2 bg-white"
          :class="bulletClass(step.step)">
          <img :src="step.icon" :alt="step.label" class="h-4 w-4 object-contain" />
          <div v-if="step.step < currentStep || status === 'done'"
            class="absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-primary-500 shadow-sm">
        ...
        <div class="flex-1 rounded-2xl px-3 py-2.5 text-left" :class="cardClass(step.step)">
          <p v-if="step.step === currentStep"
            class="mb-0.5 text-xs font-medium uppercase tracking-[0.18em] text-info-500">
            ขั้นตอนปัจจุบัน
          </p>
          ...
          <p v-if="stepDescription(step.step)"
            class="mt-0.5 text-xs leading-snug text-secondary-500 whitespace-pre-line">
...
import { Hourglass, Check } from "lucide-vue-next";
...
const bulletClass = (stepNumber: number) => {
  ...
  if (stepNumber === props.currentStep) {
    return "bg-info-50 border border-info-200 shadow-sm";
  }
  return "border-secondary-200 bg-secondary-50 opacity-80";
};
...
const cardClass = (stepNumber: number) => {
  ...
  if (stepNumber === props.currentStep) {
    return "bg-info-50 border border-info-100";
  }
  return "bg-secondary-50 border border-secondary-100 opacity-90";
};
