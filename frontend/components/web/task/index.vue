<template>
    <div class="min-h-full flex flex-col bg-[var(--bg-surface)]">
        <!-- Header & Filters -->
        <div class="px-6 py-5 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)]/95 backdrop-blur">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-[var(--text-primary)]">รายการแจ้งซ่อม</h1>
                    <p class="text-sm text-[var(--text-secondary)] mt-1">ติดตามสถานะและประวัติการแจ้งซ่อมทั้งหมด</p>
                </div>

                <div
                    class="flex items-center gap-3 bg-[var(--color-secondary-50)] p-1 rounded-xl border border-[var(--border-subtle)]">
                    <button type="button" @click="taksFilter.setActiveFilter('in_progress')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2" :class="taksFilter.activeFilter === 'in_progress'
                            ? filterStyles.in_progress
                            : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'">
                        <Clock class="w-4 h-4" />
                        <span>กำลังดำเนินการ</span>
                        <span class="ml-1 px-1.5 py-0.5 rounded-full bg-white/20 text-current text-xs">
                            {{ inProgressCount }}
                        </span>
                    </button>

                    <button type="button" @click="taksFilter.setActiveFilter('done')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2" :class="taksFilter.activeFilter === 'done'
                            ? filterStyles.done
                            : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'">
                    ...
                    <button v-if="!isEmployee" type="button" @click="taksFilter.setActiveFilter('all')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2" :class="taksFilter.activeFilter === 'all'
                            ? filterStyles.all
                            : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'">
        ...
        <!-- Content -->
        <div class="p-6">
            <div v-if="loading" class="flex flex-col items-center justify-center h-64 text-[var(--text-secondary)]">
                <Loader2 class="w-8 h-8 animate-spin mb-2 text-[var(--color-primary-500)]" />
                <p>กำลังโหลดข้อมูล...</p>
            </div>
            ...
            <div v-else class="grid grid-cols-1 gap-4">
                ...
                <!-- Task Items -->
                <NuxtLink v-for="task in visibleTasks" :key="task.id"
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-[var(--color-primary-600)]">#{{ task.ticket }}</span>
                            <span v-if="task.isMine && isCustomer"
                        ...
                        <div v-if="task.schoolName" class="text-xs text-[var(--color-info-600)] mt-0.5">
                            {{ task.schoolName }}
                        </div>
                    </div>
                    ...
                    <!-- Description -->
                    <div class="min-w-0">
                        <p class="text-sm text-[var(--text-primary)] line-clamp-2">{{ task.description }}</p>
                        <div class="flex items-center gap-3 mt-2">
                            ...
                            <!-- Rating -->
                            <div v-if="task.canRate" class="flex items-center gap-1 text-xs" :class="task.rating
                                ? 'text-[var(--color-warning-600)]'
                                : 'text-[var(--text-secondary)]'">
                                <Star class="w-3.5 h-3.5" :class="task.rating
                    ...
                    <div>
                        <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border"
                            ...
                            <span v-if="task.status === 'in_progress' && task.currentStep"
                    ...
                    <!-- Date -->
                    <div class="text-sm">
                        <div class="text-[var(--text-primary)]">{{ formatUpdatedAt(task.updatedAt) }}</div>
                        <div v-if="task.dueDate" class="text-xs text-[var(--color-error-500)] font-medium mt-1">
                            ครบกำหนด: {{ formatDueDate(task.dueDate) }}
                        </div>
                    </div>

                    <!-- Dealer Status Display -->
                    <div v-if="isDealer" class="text-xs font-semibold" :class="getDealerStatusDisplay(task).colorClass">
                ...
                <div ref="infiniteScrollTrigger" v-show="!loading && canLoadMore && filteredTasks.length > 0"
...
watch([infiniteScrollTrigger, () => filteredTasks.value.length, loading], async ([trigger, tasksLength, isLoading]) => {
