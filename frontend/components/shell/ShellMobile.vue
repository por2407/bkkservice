<template>
  <div class="relative w-full mx-auto bg-white shadow-xl flex flex-col overflow-hidden"
    ...
    <nav class="absolute bottom-0 left-0 w-full">
      <div
        class="w-full bg-primary-500 border-t border-secondary-200 shadow-[0_-10px_30px_rgba(6,95,70,0.28)] rounded-t-3xl px-3 pt-2 pb-3 text-primary-50"
        ...
        <ul class="flex items-stretch justify-between">
          <li v-for="(item, index) in sidebarMenu" :key="item.to" class="flex-1">
            <!-- internal route -->
            <NuxtLink v-if="!item.external" :to="item.to" @click="handleNavClick(index, item)"
              ...
              <span class="absolute inset-x-1 inset-y-1 rounded-xl transition-all duration-200 z-0 pointer-events-none"
              ...
              <div class="relative z-10 inline-flex items-center justify-center">
                <component :is="item.icon"
                  ...
                  :class="nav.activeIndex === index
                    ...
                    " />
                <!-- underline indicator -->
                <span class="absolute -bottom-1 h-0.5 w-5 rounded-full transition-all duration-200" :class="nav.activeIndex === index
                  ? 'bg-white/90 opacity-100 scale-100'
                  : 'opacity-0 scale-50'
                  " />
                <!-- badge -->
                <span v-if="item.badge"
                  class="absolute -top-2.5 -right-3 min-w-[16px] h-[16px] px-1 rounded-full bg-error-500 text-[9px] leading-[13px] text-white flex items-center justify-center shadow ring-[1.5px] ring-white/90">
                  {{ item.badge }}
                </span>
              </div>
